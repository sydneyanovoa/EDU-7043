...
describe(NSSI2$`2012`, `2013`, `2014`)

#Anova
NOVOA<-aov(NSSI2$`2013`~as.factor(NSSI2$Urbanization))
summary(NOVOA)

NOVOA2<-aov(NSSI2$`2013`~as.factor(NSSI2$TXRegions))
summary(NOVOA2)

NOVOA3<-aov(NSSI2$`2013`~as.factor(NSSI2$Gender))
summary(NOVOA3)

#trying to do a scatterplot
library("ggpubr")
...
ggscatter(NSSI2, x="Urbanization", y="2013",
          add="reg.line", conf.int= TRUE,
          cor.coef= TRUE, cor.method="pearson",
          xlab="Urbanization of County", ylab= "Deaths Rate per County")

ggscatter(NSSI2, x="2013", y="Urbanization",
add="reg.line", conf.int= TRUE,
cor.coef= TRUE, cor.method="pearson",
xlab="Deaths Rate per County", ylab= "Urbanization of County")
 hist(NSSI$`2013`)
...
attach(NSSI2)
t.test(NSSI2$Gender==1, NSSI2$Gender==2)

t.test(NSSI2$Urbanization==1, NSSI2$Urbanization==2)

NOVOA<-aov(NSSI2$`2013`~as.factor(NSSI2$TXRegions))
summary(NOVOA)
TukeyHSD(NOVOA)

sunflowerplot(NSSI2$`2013`, NSSI2$Urbanization)
boxplot(NSSI2$Urbanization)
boxplot(NSSI2$`2013`, NSSI2$Urbanization)
plot(NSSI2$`2013`, NSSI2$Urbanization)
plot(NSSI2$`2013`, NSSI2$TXRegions)
plot(NSSI2$Urbanization, NSSI2$`2013`)
names(NSSI2)
cor(NSSI2[18:22])
cor(NSSI2$`2013`, NSSI2$TXRegions, method = "pearson")
cor(NSSI2$`2013`, NSSI2$TXRegions, method = "spearman")
cor.test(NSSI2$`2013`, NSSI2$TXRegions, method = "spearman")
attach(NSSI2)
Correlation<-cor.test(NSSI2$"2013", NSSI2)
scatter.hist(NSSI2$`2013`,NSSI2$TXRegions)

NO<-aov(NSSI$`2013`~as.factor(NSSI$RUCC_2013))
summary(NO)

#Statistically significant (nope)
TukeyHSD(NO)
TukeyHSD(NOVOA)
