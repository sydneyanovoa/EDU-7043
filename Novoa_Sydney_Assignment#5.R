#Import dataset
library(readr)
Death_Self_Harm <- read_csv("Desktop/Death Self Harm.csv", 
...
#Create tables
table(NSSI2$Gender, NSSI2$Urbanization)
ftable(NSSI2$Gender, NSSI2$Urbanization)

table(NSSI2$Gender, NSSI2$TXRegions)
ftable(NSSI2$Gender, NSSI2$TXRegions)

#Contingency table
CrossTable(NSSI2$Gender, NSSI2$Urbanization, prop.chisq = F)
CrossTable(NSSI2$Gender, NSSI2$Urbanization, expected = T)

CrossTable(NSSI2$Gender, NSSI2$TXRegions, expected = T)
CrossTable(NSSI2$Gender, NSSI2$TXRegions, prop.chisq = F)


#Chi square
chisq.test(NSSI2$Gender, NSSI2$TXRegions)
chisq.test(NSSI2$Gender, NSSI2$Urbanization)
